...
class Snap_bangunan extends CI_Controller
{
    ...
    public function token()
    {
        ...
        $jmlbangunan = $this->input->post('jml_bangunan');
        $total = $this->input->post('total_bangunan');
        ...
        for ($i = 0; $i < count($bulan); $i++) {
            ...
            $desc = "Pembayaran Bangunan " . $nmbulan . " " . date("Y");
            $details = array(
                'id' => "$idbln",
                'price' => $jmlbangunan,
                'quantity' => 1,
    ...
    public function cekStatusTransaksi($idtrx, $nis, $orderid)
    {
        try {
            ...
            if ($response->status_code == 200) {
                $sql = "UPDATE pembayaran_bangunan SET status='0' WHERE id_transaksi='$idtrx' AND nis='$nis' AND order_id='$orderid'";
                $this->db->query($sql);
                echo 'success';
            } else {
                if ($response->transaction_status == 'pending') {
                } else {
                    $sql = "UPDATE pembayaran_bangunan SET status='1' WHERE id_transaksi='$idtrx' AND nis='$nis' AND order_id='$orderid'";
                    $this->db->query($sql);
                }
