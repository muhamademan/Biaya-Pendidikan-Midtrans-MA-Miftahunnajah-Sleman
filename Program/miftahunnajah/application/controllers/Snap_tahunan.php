...
class Snap_tahunan extends CI_Controller
{
    ...
    public function token()
    {
        ...
        $jmltahunan = $this->input->post('jml_tahunan');
        $total = $this->input->post('total_tahunan');
        ...
        for ($i = 0; $i < count($bulan); $i++) {
            ...
            $desc = "Pembayaran Tahunan " . $nmbulan . " " . date("Y");
            $details = array(
                'id' => "$idbln",
                'price' => $jmltahunan,
                'quantity' => 1,
    ...
    public function cekStatusTransaksi($idtrx, $nis, $orderid)
    {
        try {
            ...
            if ($response->status_code == 200) {
                $sql = "UPDATE pembayaran_tahunan SET status='0' WHERE id_transaksi='$idtrx' AND nis='$nis' AND order_id='$orderid'";
                $this->db->query($sql);
                echo 'success';
            } else {
                if ($response->transaction_status == 'pending') {
                } else {
                    $sql = "UPDATE pembayaran_tahunan SET status='1' WHERE id_transaksi='$idtrx' AND nis='$nis' AND order_id='$orderid'";
                    $this->db->query($sql);
                }
