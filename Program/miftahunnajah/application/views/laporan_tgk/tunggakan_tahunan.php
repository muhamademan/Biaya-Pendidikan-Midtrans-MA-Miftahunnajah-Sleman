...
<div class="container-fluid">
            ...
            <div class="container">
                ...
                <div class="form-group" id="form-nis">
                    ...
                    <button id="cari-tunggakan" class="btn btn-success"
                        onclick="javascript: window.location = '<?= base_url('laporan_tunggak/tunggakan_thn?id_siswa=') ?>'+$('#siswa').val();"><i
                            class="fas fa-search"> Tampilkan</i></button>
                    <a href="<?php echo base_url('laporan_tunggak/tunggakan_thn'); ?>"
    ...
    <div style="<?= (isset($_GET['id_siswa']) ? 'display: block;' : 'display: none;') ?>">
        ...
        <center>
            ...
            <h4>Laporan Tunggakan Pembayaran Tahunan</h4>
        </center>
        <hr>
        <table class="table table-bordered mt-5">
            ...
            <tbody>
                ...
                $sql_tunggak = "SELECT z.* FROM 
                    (
                        SELECT g.*, s.nama, ta.besar_tahunan, '1' AS jenis FROM 
                        (
                            SELECT a.id_tahunan AS id_pem, a.id_siswa, a.tahun_ajaran, a.jenis_pembayaran, f.total_tahunan AS total_bayar, IFNULL(f.jml_bulan, 0) AS jml_bulan, IF(f.jml_bulan = 1, 'Lunas', 'Belum Lunas') AS status_bayar 
                ...
                foreach ($result as $v) {
                    ...
                    echo '<tr>
							...
							<td style="text-align: right;"> Rp. ' . number_format(($v['jenis'] == '1' && $v['jml_bulan'] == 0 ? 1 * $v['besar_tahunan'] : ($v['jenis'] == '1' && $v['jml_bulan'] > 0 ? (1 * $v['besar_tahunan']) - $v['total_bayar'] : $v['total_bayar'])), 0, ',', '.') . '</td>
