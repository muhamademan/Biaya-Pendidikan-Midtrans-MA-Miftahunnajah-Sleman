                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <?php echo anchor('siswa_bulanan/hapus_spp/' . $u->id_transaksi . '/' . $u->id_siswa . '/' . $id_spp, '<input type=reset class="btn btn-outline-danger btn-sm" value=\'Hapus\'>'); ?>
                                        <?php } ?>

                                        <!-- cetak kwitansi pembayaran spp -->
                                        <?php echo anchor('siswa_bulanan/kwitansi_Spp/' . $u->nis . '/' . $u->id_siswa, 'Kwitansi', array('title' => 'Cetak Kwitansi Pembayaran', 'class' => 'btn btn-outline-info btn-sm')); ?>
                                        <br>
    ...
    <div class="col-12 col-md-3">
        <div class="card">
            ...
            <div class="card-content">
                ...
                <form id="form-spp" class="form-inline" method="post"
                    action="<?= base_url('siswa_bulanan/tambah_aksi/'); ?>">
                    ...
                    <input name="id_spp" class="form-control" type="text" value="<?php echo $id_spp; ?>" hidden>
                    ...
                    <div class="form-group col-12">
                        ...
                        <select name="tahun_ajaran" id="tahun_ajaran" class="form-control" required>
                            ...
                            <?php
                                ...
                                ?>
                            <option name="besar_spp" id="besar_spp" value="<?php echo $tahun->id_tahun ?>">
                                <?php echo $tahun->tahun_ajaran . ' | Rp. ' . number_format($tahun->besar_spp, 0, ',', '.'); ?>
                            </option>
                            <?php } ?>
                        </select>
                    </div>
                    ...
                    &nbsp;<a href="<?= base_url('siswa_bulanan/spp'); ?>" class="btn btn-danger mb-2"><i
...
<script src="https://app.sandbox.midtrans.com/snap/snap.js" data-client-key="SB-Mid-client-TiU0qqjgdH3mFdTD"></script>
<script type="text/javascript">
//S: hendi
...
$(document).ready(function() {
    ...
    $("#form-spp").submit(function(e) {
        ...
        if ($('#metode-pembayaran').val() == 'Online' && _issubmit === false) {
            ...
            var _jmlspp = $('#tahun_ajaran option:selected').text().split('|')[1].replace('Rp. ', '')
                .replace('.', '');
            var _total = hitungTotalSPP();
            //alert('a');exit;
            $.ajax({
                type: 'POST',
                url: '<?= base_url() ?>snap_spp/token',
                data: {
                    nis: _nis,
...
function cekStatusTransaksi(_idtransaksi, _nis, _orderid) {
    $.ajax({
        type: 'POST',
        url: '<?= base_url() ?>snap_spp/cekStatusTransaksi/' + _idtransaksi + '/' + _nis + '/' + _orderid,
        data: {},
        cache: false,
