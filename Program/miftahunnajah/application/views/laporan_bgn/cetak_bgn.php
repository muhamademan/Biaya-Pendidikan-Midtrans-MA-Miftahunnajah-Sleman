...
$pdf->MultiCell(31, 0.7, "DATA PEMBAYARAN BIAYA BANGUNAN SISWA MA MIFTAHUNNAJAH SLEMAN", 0, 'C');
...
if (!empty($pembayaran_bangunan)) {
    $no = 1;
    foreach ($pembayaran_bangunan as $data) {
        $pdf->SetFont('Arial', '', 10);
        ...
        $pdf->Cell(3, 0.6, 'Rp. ' . number_format($data->besar_bangunan, 0, ',', '.'), 1, 0, 'C');
        $pdf->ln();
    }
    // BAGIAN TOTAL PEMBAYARAN BIAYA BANGUNAN
    $pdf->SetFont('Arial', 'B', 10);
    $pdf->Cell(25, 0.8, 'TOTAL PEMBAYARAN BIAYA BANGUNAN', 1, 0, 'C');
...
$pdf->Output("Laporan Pembayaran Bangunan.pdf", "I");